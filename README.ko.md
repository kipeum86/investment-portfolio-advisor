...
```
사용자 입력 ("5000만원, 1년, 중립적")
    │
    ▼
┌─────────────────────────────────────────────────────────┐
│  Step 0   신선도 체크              → REUSE / FULL 라우팅 │
│  Step 1   쿼리 인터프리터          → user-profile.json   │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ══ Phase 1: 투자 환경 평가 (순차 실행) ══               │
│                                                         │
│  Step 2   거시경제 수집기          → macro-raw.json      │
│  Step 3   정치/지정학 수집기       → political-raw.json  │
│  Step 4   펀더멘탈 수집기          → fundamentals-raw.json│
│  Step 5   시장심리 수집기          → sentiment-raw.json  │
│                                                         │
│  Step 6   데이터 검증     [스크립트] → validated-data.json│
│  Step 7   환경 스코어링   [LLM]    → env-assessment.json │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ══ Phase 2: 포트폴리오 구성 ══                          │
│                                                         │
│  Step 8   포트폴리오 애널리스트 [에이전트] → 3개 옵션    │
│  Step 9   퀄리티 게이트   [스크립트] → 7개 결정론적 체크 │
│  Step 10  비평가 리뷰     [에이전트] → 정성적 평가       │
│  Step 11  산출물 생성               → HTML + DOCX       │
│  Step 12  영속화                    → 아카이브 + latest  │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
